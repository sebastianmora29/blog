<x-layouts.app :title="__('Categorias')">
    ...
    <div class="w-full p-4 bg-[#171717] border border-gray-700 rounded-lg shadow-sm text-white">
        ...
        <form class="w-full" action="{{ route('admin.categorias.update', $categoria) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <input
                    type="text" id="nombre" name="nombre" value="{{ old('nombre', $categoria->nombre) }}" class="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 border rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-500 {{ $errors->has('nombre') ? 'border-red-500' : 'border-gray-300 dark:border-gray-600' }}" placeholder="Ingresa una categoría" required />
                @error('nombre')
